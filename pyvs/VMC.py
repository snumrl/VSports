...
class SLAC(object):
	...
	def saveModel(self):
		self.scheduler_model[0].save('../nn/current.pt')

		if self.scheduler_max_return_epoch == self.num_evaluation:
			self.scheduler_model[0].save('../nn/max.pt')
		if self.num_evaluation%20 == 0:
			self.scheduler_model[0].save('../nn/'+str(self.num_evaluation)+'.pt')



	def loadlModel(self,path,index):
		self.scheduler_model[index].load('../nn/'+path+'.pt')
	...
	def generateTransitions(self):
		...
		for i in range(self.num_slaves):
			for j in range(self.num_agents):
				scheduler_states[i*self.num_agents+j] = self.env.getState(i,j)
			...
			for i in range(self.num_slaves):
				...
				for k in range(self.num_agents):
					if teamDic[k] == learningTeam:
						scheduler_rewards[i*self.num_agents+k] = self.env.getReward(i, k)
						if np.any(np.isnan(scheduler_rewards[i*self.num_agents+k])):
							nan_occur = True
	...
	def evaluate(self):
		...
		print('=============================================')
		return np.array(self.scheduler_rewards)
...
if __name__=="__main__":
	...
	for i in range(5000000):
		...
		plot(scheduler_rewards, graph_name + ' reward',0,False)
