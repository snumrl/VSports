...
class SLAC(object):
	def __init__(self):
		...
		for i in range(self.num_slaves*self.num_agents):
			# exit()
			self.scheduler_model[i] = SchedulerNN(self.num_state)
	...
	def generateTransitions(self):
		...
		while True:
			...
			for i in range(self.num_slaves):
				...
				for j in range(self.num_agents):
					...
					if useHardCoded :
						if teamDic[j] == learningTeam:
							...
							exploitationFlag = random.randrange(0,2)
							if exploitationFlag == 0:
								scheduler_actions[i*self.num_agents+j] = scheduler_a_dist_slave[j].sample().cpu().detach().numpy()[0][0];
							else :
				...
				for j in range(self.num_agents):
					if teamDic[j] == learningTeam:
						# print(Tensor(lactor_states[i*self.num_agents+j]).size())
						# print(Tensor(scheduler_actions[i*self.num_agents+j]).size())
						# np.concatenate((lactor_states[i*self.num_agents+j], scheduler_actions[i*self.num_agents+j]), axis=0)
...
if __name__=="__main__":
	...
	for i in range(5000000):
		...
		plot(scheduler_rewards,'scheduler reward',0,False)
