...
class PPO(object):
	def __init__(self):
		...
		self.episodes = [[0 for x in range(self.num_agents)]]*self.num_slaves
		for j in range(self.num_slaves):
			# self.episodes[j] = []*self.num_agents
			for k in range(self.num_agents):
				self.episodes[j][k] = EpisodeBuffer()
		self.env.resets()
	...
	def computeTDandGAE(self):
		...
		for epi in self.total_episodes:
			...
			if not np.isnan(epi_return):
				...
				rnn_replay_buffer = RNNReplayBuffer(1000)
	...
	def optimizeSimulationNN(self):
		...
		for j in range(self.num_epochs):
			...
			for rnn_replay_buffer in all_rnn_replay_buffer:
				...
				# Two cases, 1 : the replay buffer size is bigger than trunc, 2 : o/w.
				# In both cases, we will also include the remainder part 
				# (whose length are smaller than self.trunc_size)
				if rnn_replay_buffer_size >= self.trunc_size:
					...
					if rnn_replay_buffer_size%self.trunc_size != 0:
						segmented_transitions = np.array(rnn_replay_buffer.buffer)[rnn_replay_buffer_size//self.trunc_size * self.trunc_size:]
				else :
					segmented_transitions = np.array(rnn_replay_buffer.buffer)
					all_segmented_transitions.append(segmented_transitions)
			...
			for i in range(len(all_segmented_transitions)//self.batch_size):
				...
				# print(len(segmented_transitions[0]))
				# print(*zip(*segmented_transitions))
				# batch = RNNTransition(*zip(*segmented_transitions))

				for timeStep in range(self.trunc_size):

					transition_for_batch = [None]*self.batch_size
					for k in range(self.batch_size):
						# segmented_transition = batch_segmented_transitions[k]
						transition_for_batch[k] = batch_segmented_transitions[k][timeStep]

					batch = Transition(*zip(*transition_for_batch))
